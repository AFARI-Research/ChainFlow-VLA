...
def pdm_score_with_details(
    metric_cache: MetricCache,
    model_trajectory: Trajectory,
    future_sampling: TrajectorySampling,
    simulator: PDMSimulator,
    scorer: PDMScorer,
) -> Tuple[PDMResults, Dict[str, Any]]:
    """
    Same as :func:`pdm_score`, but also returns intermediate arrays and scorer
    internals needed by InternVL per-token evaluation artifacts.
    """
    ...
    details: Dict[str, Any] = {
        "pred_idx": pred_idx,
        "pdm_states": pdm_states,
        "pred_states": pred_states,
        "simulated_states_pred": simulated_states[pred_idx],
        "score": float(score),
        "no_at_fault_collisions": float(no_at_fault_collisions),
        "drivable_area_compliance": float(drivable_area_compliance),
        "ego_progress": float(ego_progress),
        "time_to_collision_within_bound": float(time_to_collision_within_bound),
        "comfort": float(comfort),
        "driving_direction_compliance": float(driving_direction_compliance),
        "progress_raw_m": float(scorer._progress_raw[pred_idx]),
        "ego_area_multiple_lanes": scorer._ego_areas[pred_idx, :, EgoAreaIndex.MULTIPLE_LANES].tolist(),
        "ego_area_non_drivable_area": scorer._ego_areas[pred_idx, :, EgoAreaIndex.NON_DRIVABLE_AREA].tolist(),
        "ego_area_oncoming_traffic": scorer._ego_areas[pred_idx, :, EgoAreaIndex.ONCOMING_TRAFFIC].tolist(),
        "collision_time_idx": float(scorer._collision_time_idcs[pred_idx]),
        "ttc_time_idx": float(scorer._ttc_time_idcs[pred_idx]),
    }
