...
from navsim.common.dataclasses import Annotations, Camera, Lidar, Trajectory
...
def add_trajectory_to_camera_ax(ax: plt.Axes, camera: Camera, trajectory: Trajectory, config: Dict[str, Any]) -> plt.Axes:
    """
    Adds trajectory to camera image by projecting local xy poses into the image plane.
    """
    poses_2d = trajectory.poses[:, :2]
    poses_3d = np.concatenate([poses_2d, np.zeros((poses_2d.shape[0], 1), dtype=poses_2d.dtype)], axis=1)
    poses = np.concatenate([np.array([[0, 0, 0]], dtype=poses_3d.dtype), poses_3d], axis=0)

    pc_in_cam, pc_in_front_mask = _transform_pcs_to_images(
        poses.T,
        camera.sensor2lidar_rotation,
        camera.sensor2lidar_translation,
        camera.intrinsics,
    ...
    visible_segments = []
    for idx in range(len(pc_in_cam) - 1):
        if not (pc_in_front_mask[idx] or pc_in_front_mask[idx + 1]):
            continue

        clipped_segment = _clip_line_to_image_bounds(
            pc_in_cam[idx],
            pc_in_cam[idx + 1],
            image_width,
            image_height,
    ...
    for start_point, end_point in visible_segments:
        segment_points = np.stack([start_point, end_point], axis=0)
        ax.plot(
            segment_points[:, 0],
            segment_points[:, 1],
...
def _clip_line_to_image_bounds(
    start_point: npt.NDArray[np.float32],
    end_point: npt.NDArray[np.float32],
    image_width: int,
    image_height: int,
) -> Optional[Tuple[npt.NDArray[np.float32], npt.NDArray[np.float32]]]:
    """Clip a line segment into the image rectangle with Liang-Barsky."""
    ...
    clipped_start = np.array([x1 + u1 * dx, y1 + u1 * dy], dtype=np.float32)
    clipped_end = np.array([x1 + u2 * dx, y1 + u2 * dy], dtype=np.float32)
