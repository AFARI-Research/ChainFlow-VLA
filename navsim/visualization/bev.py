...
def _ego_box_config_from_traj_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """BEV ego footprint style derived from trajectory line style."""
    box_config = dict(AGENT_CONFIG[TrackedObjectType.EGO])
    color = config.get("line_color", box_config["line_color"])
    box_config["fill_color"] = color
    box_config["line_color"] = color
    box_config["fill_color_alpha"] = 0.35
    box_config["zorder"] = int(config.get("zorder", 2)) + 1
    return box_config


def add_ego_box_at_local_pose_to_bev_ax(
    ax: plt.Axes,
    pose: np.ndarray,
    ...
    """
    Draw Pacifica ego oriented box at a rear-axle pose in ego-local BEV coordinates.

    :param pose: (x, y, heading) in the same local frame as trajectory poses.
    """
    if config is None:
        config = AGENT_CONFIG[TrackedObjectType.EGO]
    car_footprint = CarFootprint.build_from_rear_axle(
        rear_axle_pose=StateSE2(float(pose[0]), float(pose[1]), float(pose[2])),
        vehicle_parameters=get_pacifica_parameters(),
    )
    return add_oriented_box_to_bev_ax(
        ax, car_footprint.oriented_box, config, add_heading=add_heading
    )
...
def add_trajectory_to_bev_ax(
    ...
    config: Dict[str, Any],
    draw_ego_box_at_end: bool = False,
) -> plt.Axes:
    """
    ...
    :param draw_ego_box_at_end: if True, draw ego bbox at the last trajectory pose
    :return: ax with plot
    """
    ...
    if draw_ego_box_at_end and trajectory.poses.shape[0] > 0:
        box_config = _ego_box_config_from_traj_config(config)
        add_ego_box_at_local_pose_to_bev_ax(ax, trajectory.poses[-1], box_config, add_heading=True)
    return ax
...
def add_oriented_box_to_bev_ax(
    ...
    fill_zorder = config["zorder"]
    line_zorder = fill_zorder + 0.1
    heading_zorder = fill_zorder + 0.2
