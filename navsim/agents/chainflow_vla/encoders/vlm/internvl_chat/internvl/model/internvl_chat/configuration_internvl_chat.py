    ...
    def __init__(
        ...
        if isinstance(llm_config, dict):
            ...
            if not arch_name:
                ...
                arch_name = arch_by_model_type.get(model_type, '')
                if arch_name:
                    llm_config['architectures'] = [arch_name]
                    logger.warning(
                        f'llm_config.architectures is missing/empty, inferred architecture from model_type={model_type}: {arch_name}'
                    )
        if not arch_name:
            arch_name = 'LlamaForCausalLM'
            llm_config['architectures'] = [arch_name]
            logger.warning(
                'llm_config.architectures is missing/empty and model_type is unknown. Fallback to LlamaForCausalLM.'
            )

        self.vision_config = InternVisionConfig(**vision_config)
        if arch_name == 'LlamaForCausalLM':
            self.llm_config = LlamaConfig(**llm_config)
        elif arch_name == 'InternLM2ForCausalLM':
            self.llm_config = InternLM2Config(**llm_config)
        elif arch_name == 'Phi3ForCausalLM':
            self.llm_config = Phi3Config(**llm_config)
        elif arch_name == 'Qwen2ForCausalLM':
