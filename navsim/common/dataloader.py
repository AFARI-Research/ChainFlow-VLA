...
class SceneLoader:
    ...
    def _build_token_to_log_file(self) -> None:
        """Build the token-to-log mapping used by distributed InternVL evaluation."""
        for token, scene_dict_list in self.scene_frames_dicts.items():
            self.token_to_log_file[token] = scene_dict_list[0]["log_name"]
