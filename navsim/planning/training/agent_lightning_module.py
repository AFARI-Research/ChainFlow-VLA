...
class AgentLightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch: Tuple[Dict[str, Tensor], Dict[str, Tensor]], batch_idx: int):
        ...
        :return: scalar loss
        """
        if 'chainflow_vla' in self.agent.name() or "ChainFlowVLA" in self.agent.name():
            features, targets = batch
    ...
    def predict_step(self, batch: Tuple[Dict[str, Tensor], Dict[str, Tensor]], batch_idx: int):
        ...
        """
        return self.predict_step_chainflow_vla(batch, batch_idx)

    def predict_step_chainflow_vla(self, batch: Tuple[Dict[str, Tensor], Dict[str, Tensor], List[str]], batch_idx: int):
