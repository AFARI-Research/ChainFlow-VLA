...
def main(cfg: DictConfig) -> None:
    ...
    num_successful_scenarios = int(pdm_score_df[pdm_score_df["token"] != "average"]["valid"].sum())
    num_failed_scenarios = int(len(pdm_score_df) - 1 - num_successful_scenarios)
    average_score = average_row.get("score", float("nan"))
    summary = (
        f"Submission PDM score: {average_score} | "
        f"successful={num_successful_scenarios} failed={num_failed_scenarios} | "
        f"csv={csv_path}"
    )
    logger.info(summary)
    print(summary)
